...
class FireStoreRepo {
    ...
    fun getRestaurant() =
        Single.create<List<RestaurantData>> { emitter ->
            firestoreDataSource.getRestaurantData(emitter)
        }
            .subscribeOn(Schedulers.io())
            .toObservable()


    fun getNearbySchoolRestaurant(searchText: String) =
        Single.create<List<RestaurantData>> { emitter ->
            firestoreDataSource.getNearbySchoolRestaurant(searchText,emitter)
        }
            .subscribeOn(Schedulers.io())
            .toObservable()
